import unittest
from language import Language, Grammar, Vocabulary
...
class TestRuleGenerator(unittest.TestCase):
    ...
    # def test_generate_random_rule(self):
    #     rule_generator = RuleGenerator(language=self.language, Rlen=2, Rsample='random')

    #     with open("generated_rules.txt", "w") as f:
    #         for _ in range(100):  # Generate 100 rules
    #             rule = rule_generator.generate_rule()
    #             f.write(rule + "\n")

    def test_generate_llm_guided_rule(self):
        rule_generator = RuleGenerator(language=self.language, Rlen=2, Rsample='llm_guided')
        rule = rule_generator.generate_llm_guided_rule()
        print("Generated LLM-guided rule:", rule)
        self.assertIsInstance(rule, str)
        self.assertTrue(rule.strip().startswith(self.language.vocab.identifiers['positive_label']))
